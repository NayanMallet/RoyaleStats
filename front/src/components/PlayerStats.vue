...
const openClanModal = async (tag: string): Promise<ClanDetails | null> => {
    if (!tag) return null
    try {
...
async function handleSearch(tagInput: string): Promise<PlayerProfile | null> {
...
async function handleClanSearch(tagInput: string): Promise<ClanDetails | null> {
    if (!tagInput) return null
    let tag = tagInput.trim().toUpperCase()
    if (!tag.startsWith('#')) tag = '#' + tag

    // Use existing logic to open clan modal
    return await openClanModal(tag)
}
...
<template>
    <div class="flex flex-col gap-12 w-full max-w-7xl mx-auto font-sans pb-24 px-4 sm:px-6">
        ...
        <div v-if="player" class="animate-in fade-in slide-in-from-bottom-12 duration-1000 space-y-12">
            ...
            <!-- Hero Section -->
            <div
                class="relative overflow-hidden rounded-[2.5rem] bg-slate-900 text-white shadow-2xl shadow-slate-900/20 group">
                <!-- Hero Background -->
                <div class="absolute inset-0 bg-gradient-to-r from-slate-900 via-slate-900/90 to-slate-900/40 z-10">
                </div>
                <!-- Arena Image (Right Side) -->
                <img v-if="player.arena" :src="getArenaDetails(player.arena.id, player.arena.name).image"
                ...
                <img v-if="heroImage" :src="heroImage"
                    class="absolute right-12 top-1/2 -translate-y-1/2 w-1/2 md:w-1/3 h-auto object-contain opacity-20 blur-sm md:blur-0 mix-blend-overlay group-hover:scale-105 transition-transform duration-1000 rotate-12" />

                <div class="relative z-20 p-8 md:p-16 flex flex-col md:flex-row items-center md:items-start gap-10">
                    <!-- Avatar -->
                    <div class="relative shrink-0">
                        <div class="absolute inset-0 bg-yellow-400 rounded-full blur-2xl opacity-20 animate-pulse">
                        </div>
                        <div
                            class="bg-gradient-to-b from-yellow-300 to-yellow-600 w-28 h-28 md:w-36 md:h-36 rounded-[2rem] flex items-center justify-center shadow-[0_0_40px_-10px_rgba(234,179,8,0.6)] border-4 border-yellow-200 relative transform rotate-3 hover:rotate-6 transition-transform">
                            <span class="text-5xl md:text-6xl font-black text-white drop-shadow-lg">{{ player.expLevel
                                }}</span>
                        </div>
                        <div
                            class="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur border border-white/20 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest text-yellow-300">
                            Niveau
                        </div>
                    </div>
                    ...
                    <div class="flex-1 text-center md:text-left space-y-4">
                        <div>
                            <h1
                                class="text-5xl md:text-7xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-400 mb-2">
                                {{ player.name }}
                            </h1>
                            <div class="flex flex-wrap items-center justify-center md:justify-start gap-3">
                                <span
                                    class="text-slate-400 font-mono font-bold text-lg bg-slate-800/50 px-3 py-1 rounded-lg border border-slate-700/50">{{
                                        player.tag }}</span>
                                <span v-if="player.clan" @click="openClanModal(player.clan.tag)"
                                    ...
                                    <Loader2 v-if="loadingClan && selectedClan?.tag === player.clan.tag"
                                    ...
                                    <!-- Real Image -->
                                    <img v-else :src="getClanBadgeUrl(player.clan.badgeId)"
                                        class="w-5 h-5 object-contain" alt="Clan Badge" @error="handleImageError" />
                                    {{ player.clan.name }}
                                </span>
                                <span v-if="player.arena"
                                    class="flex items-center gap-1.5 text-blue-300 font-bold bg-blue-900/30 px-3 py-1 rounded-lg border border-blue-500/20">
                                    <Target class="w-4 h-4" /> {{ getArenaDetails(player.arena.id || 0,
                                        player.arena.name).name }}
                                </span>
                            </div>
                        </div>
                        ...
                        <!-- Hero Stats -->
                        <div
                            class="grid grid-cols-2 md:grid-cols-4 gap-6 pt-6 pb-2 bg-gradient-to-b from-slate-900/0 to-slate-900/80 rounded-b-[2.5rem] -mx-8 -mb-8 md:-mx-16 md:-mb-16 px-8 md:px-16 pb-8 md:pb-16 mt-4">
                            ...
                            <div class="text-center md:text-left">
                                <div class="text-3xl font-black text-white">{{ getWinRate(player.wins,
                                    player.battleCount) }}</div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div class="xl:col-span-2 space-y-8">
                    <!-- Path of Legends -->
                    <div v-if="rankedStats?.current"
                        class="bg-white rounded-3xl p-8 shadow-xl border border-slate-100 relative overflow-hidden">
                        <div
                            class="absolute top-0 right-0 w-32 h-32 bg-purple-100 rounded-bl-full opacity-50 pointer-events-none">
                        </div>
                        ...
                        <div class="grid md:grid-cols-3 gap-6 relative z-10">
                            <div
                                class="bg-purple-50/50 rounded-2xl p-4 border border-purple-100 text-center hover:bg-purple-50 transition-colors">
                                <div class="text-xs font-bold text-purple-400 uppercase tracking-wider mb-2">Saison
                                    Actuelle</div>
                                <div class="text-3xl font-black text-slate-800 mb-1">Ligue {{
                                    rankedStats.current.leagueNumber }}</div>
                                <div class="text-sm font-medium text-slate-500">{{ rankedStats.current.trophies }}
                                    Médailles</div>
                            </div>
                            <div class="bg-white rounded-2xl p-4 border border-slate-100 text-center shadow-sm">
                                <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Saison
                                    Précédente</div>
                                <div class="text-2xl font-bold text-slate-700 mb-1">Ligue {{
                                    rankedStats.last?.leagueNumber ?? '-' }}</div>
                                <div class="text-sm text-slate-400">{{ rankedStats.last?.trophies ?? 0 }} Médailles
                                </div>
                            </div>
                            <div class="bg-white rounded-2xl p-4 border border-slate-100 text-center shadow-sm">
                                <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Meilleur
                                    Résultat</div>
                                <div class="text-2xl font-bold text-slate-700 mb-1">Ligue {{
                                    rankedStats.best?.leagueNumber ?? '-' }}</div>
                                <div class="text-sm text-slate-400">{{ rankedStats.best?.trophies ?? 0 }} Médailles
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Goblin Journey -->
                    <div
                        class="bg-gradient-to-br from-emerald-900 to-emerald-800 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl">
                        ...
                        <div class="relative z-10 flex justify-between items-center">
                            ...
                            <div class="text-right">
                                <div v-if="goblinProgress">
                                    ...
                                    <div class="text-sm font-bold text-emerald-400">{{ goblinProgress.arena?.name }}
                                    </div>
                                </div>
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-white rounded-3xl p-6 shadow-xl border border-slate-100">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-xs font-bold bg-blue-50 text-blue-600 px-2 py-1 rounded-md border border-blue-100">
                                Moy. {{(player.currentDeck.reduce((acc, c) => acc + (c.elixirCost || 0), 0) /
                                    8).toFixed(1)}}
                            </span>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            <div v-for="card in player.currentDeck" :key="card.id"
                                class="group relative aspect-[3/4] cursor-pointer" @click="openCardModal(card)">
                                <div
                                    class="absolute inset-0 bg-slate-100 rounded-xl transform rotate-3 scale-90 opacity-0 group-hover:opacity-100 group-hover:rotate-6 transition-all duration-300">
                                </div>
                                <img :src="card.iconUrls.medium"
                                    class="w-full h-full object-contain relative z-10 drop-shadow-md group-hover:-translate-y-1 transition-transform" />

                                <div
                                    class="absolute -top-1 -right-1 bg-purple-600 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold border border-white z-20 shadow-sm">
                                    {{ getElixirCost(card.name) }}
                                </div>
                                <div
                                    class="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 bg-slate-800 text-white text-[9px] px-1.5 py-0.5 rounded shadow-sm z-20 border border-slate-600 whitespace-nowrap">
                    ...
                    <!-- Badges Grid -->
                    <div v-if="filteredBadges.length"
                        class="bg-white rounded-3xl p-6 shadow-xl border border-slate-100">
                        ...
                        <div class="grid grid-cols-4 gap-4">
                            <div v-for="badge in filteredBadges.slice(0, 16)" :key="badge.name"
                                class="aspect-square bg-slate-50 rounded-xl p-2 flex items-center justify-center border border-slate-100 hover:border-blue-200 transition-colors group cursor-help relative">
                                <img :src="badge.iconUrls.large"
                                    class="w-full h-full object-contain grayscale-[0.3] group-hover:grayscale-0 transition-all group-hover:scale-110" />
                                <!-- Tooltip fallback -->
                                <div
                                    class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-30">
                                    {{ badge.name }} ({{ badge.level }})
                                </div>
                            </div>
                        </div>
                        <div v-if="filteredBadges.length > 16" class="text-center mt-4">
                            <Button @click="openBadgesModal" variant="ghost" size="sm"
                                class="text-xs text-slate-400 font-medium hover:text-slate-600">
        ...
        <!-- Badges Full Modal -->
        <div v-if="showBadgesModal"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300"
            @click.self="closeBadgesModal">
            <div
                class="bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-300">
                <!-- Header -->
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-2xl font-black text-slate-800">Tous les Badges</h3>
                    <Button variant="ghost" class="rounded-full w-10 h-10 p-0 hover:bg-slate-200 text-slate-500"
                ...
                <div class="p-8 overflow-y-auto grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-6">
                    <div v-for="badge in filteredBadges" :key="badge.name"
                        class="flex flex-col items-center gap-2 group">
                        <div
                            class="relative w-20 h-20 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 group-hover:border-blue-300 group-hover:bg-blue-50 transition-all">
                            <img :src="badge.iconUrls.large" :alt="badge.name"
                                class="w-14 h-14 object-contain drop-shadow-sm group-hover:scale-110 transition-transform" />
                            <div class="absolute -top-2 -right-2 bg-slate-900 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border border-white"
                                v-if="badge.level">
                                Lvl {{ badge.level }}
                            </div>
                        </div>
                        <div
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-tight text-center truncate w-full">
                            {{ badge.name }}</div>
                    </div>
                </div>
            </div>
        ...
        <!-- Clan Modal -->
        <div v-if="showClanModal && selectedClan"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300"
            @click.self="closeClanModal">
            <div
                class="bg-white rounded-[2rem] shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-300">
                <!-- Header -->
                <div
                    class="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row gap-6 items-center bg-slate-50/50">
                    <img v-if="getClanBadgeUrl(selectedClan.badgeId)" :src="getClanBadgeUrl(selectedClan.badgeId)"
                        class="w-24 h-24 object-contain drop-shadow-lg" />
                    <div v-else
                        class="w-24 h-24 flex items-center justify-center bg-orange-100 rounded-full border-4 border-white shadow-lg">
                        <Shield class="w-12 h-12 text-orange-500" />
                    </div>
                    <div class="flex-1 text-center md:text-left space-y-2">
                        <h3 class="text-3xl md:text-4xl font-black text-slate-800 tracking-tight">{{ selectedClan.name
                            }}</h3>
                        <div class="flex flex-wrap items-center justify-center md:justify-start gap-3">
                            <span
                                class="text-slate-400 font-mono font-bold bg-slate-200/50 px-2 py-1 rounded text-sm">{{
                                    selectedClan.tag }}</span>
                            <span
                                class="flex items-center gap-1.5 text-slate-600 font-bold text-sm bg-slate-100 px-3 py-1 rounded-full">
                    ...
                    <div class="flex gap-4">
                        <div class="text-center px-4 py-2 bg-yellow-50 rounded-xl border border-yellow-100">
                            ...
                            <div class="text-[10px] font-bold text-yellow-600 uppercase tracking-widest">Score Clan
                            </div>
                        </div>
                        <div class="text-center px-4 py-2 bg-green-50 rounded-xl border border-green-100">
                            ...
                            <div class="text-[10px] font-bold text-green-600 uppercase tracking-widest">Dons / Sem.
                            </div>
                        </div>
                    </div>
                    <Button variant="ghost"
                        class="absolute top-4 right-4 rounded-full w-10 h-10 p-0 hover:bg-slate-200 text-slate-400"
                        @click="closeClanModal">
                        <X class="w-6 h-6" />
                    </Button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-0">
                    <table class="w-full text-left border-collapse">
                        <thead
                            class="bg-slate-50 sticky top-0 z-10 shadow-sm text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="member in selectedClan.memberList" :key="member.tag"
                                ...
                                <td class="p-4">
                                    <div
                                        class="font-bold text-slate-800 group-hover:text-blue-600 transition-colors text-lg">
                                        {{ member.name }}</div>
                                    <div class="flex items-center gap-1.5 text-xs text-slate-400 md:hidden">
                                        <span>{{ member.role === 'leader' ? 'Chef' : member.role === 'coLeader' ?
                                            'Chef Adjoint' : member.role === 'elder' ? 'Aîné' : 'Membre' }}</span>
                                        <span>•</span>
                                ...
                                <td class="p-4 hidden md:table-cell">
                                    <span :class="{
                                        ...
                                        {{ member.role === 'leader' ? 'Chef' : member.role === 'coLeader' ?
                                            'Chef Adjoint'
                                            : member.role === 'elder' ? 'Aîné' : 'Membre' }}
                                    </span>
                                </td>
        ...
        <div v-if="selectedCard"
            ...
            <div
                class="bg-gradient-to-b from-slate-800 to-slate-900 rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 relative border border-slate-700/50">

                <Button variant="ghost"
                    class="absolute top-4 right-4 rounded-full w-10 h-10 p-0 hover:bg-white/10 text-white/70 hover:text-white z-20 transition-colors"
                ...
                <div class="relative h-80 flex items-center justify-center bg-slate-900">
                    ...
                    <img :src="selectedCard.iconUrls.medium"
                        class="w-64 h-auto drop-shadow-[0_10px_60px_rgba(0,0,0,0.6)] z-10 transform hover:scale-105 transition-transform duration-700 ease-in-out" />
                </div>
                ...
                <div class="p-8 space-y-6 relative z-20 -mt-16">
                    <div class="text-center space-y-2">
                        <div class="flex items-center justify-center gap-2">
                            <div
                                class="inline-flex items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full border border-white/10 backdrop-blur-md">
                            ...
                            <div v-if="selectedCard.evolutionLevel"
                                class="inline-flex items-center gap-1.5 px-3 py-1 bg-purple-500/20 rounded-full border border-purple-500/30 backdrop-blur-md text-purple-300">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div
                            class="bg-white/5 rounded-2xl p-3 text-center border border-white/5 hover:bg-white/10 transition-colors">
                        ...
                        <div
                            ...
                            <div class="text-xl font-black text-purple-400 drop-shadow-glow">{{
                                getElixirCost(selectedCard.name) }}</div>
                        </div>
                        <div
                            class="bg-white/5 rounded-2xl p-3 text-center border border-white/5 hover:bg-white/10 transition-colors">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Évolution
                            </div>
                            <div class="text-xl font-black text-slate-200">{{ selectedCard.evolutionLevel ? 'Oui' :
                                'Non' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white/5 rounded-2xl p-4 border border-white/5 space-y-3">
                        <div class="flex justify-between items-center text-xs text-slate-300">
                            ...
                            <span class="font-bold font-mono">{{ selectedCard.count }} / {{ selectedCard.maxLevel ?
                                'MAX' : '?' }}</span>
                        </div>
                        <div class="flex justify-between items-center text-xs text-slate-300">
                        ...
                        <div class="flex justify-between items-center text-xs text-slate-300">
                            ...
                            <span class="font-bold text-yellow-400">{{ selectedCard.maxLevel ? selectedCard.maxLevel + 1
                                : '15' }}</span>
                        </div>
                        <!-- Note on stats -->
