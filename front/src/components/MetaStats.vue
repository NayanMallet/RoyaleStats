...
const handleImageError = (event: Event) => {
    ...
    if (target) {
        target.src = 'https://raw.githubusercontent.com/RoyaleAPI/cr-api-assets/master/cards-75/knight.png'
    }
}
...
<template>
    <div class="w-full max-w-7xl mx-auto px-4 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="text-center space-y-2 mb-12">
            ...
            <p class="text-slate-500 font-medium text-lg">Statistiques basées sur {{ snapshot?.totalBattles || 0 }} batailles analysées</p>
        </div>
        ...
        <div v-else-if="!snapshot || snapshot.topCards.length === 0" class="flex flex-col items-center justify-center py-20 text-slate-400 bg-white/50 backdrop-blur-sm rounded-3xl border border-dashed border-slate-300">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="(card, index) in snapshot.topCards" :key="card.card_name" 
                ...
                <div class="p-4 flex items-center gap-4">
                    <div class="relative w-16 h-20 flex-shrink-0">
                         <img :src="getCardUrl(card.card_name)" 
                              @error="handleImageError"
                              class="w-full h-full object-contain drop-shadow-md group-hover:scale-110 transition-transform duration-300" />
                         <div class="absolute -top-2 -left-2 w-6 h-6 bg-slate-800 text-white rounded-full flex items-center justify-center text-xs font-black border-2 border-white shadow-sm">
                             {{ index + 1 }}
                         </div>
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-4 mt-1">
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wide">Usage</span>
                                <span class="font-bold text-slate-600">{{ card.usage_count }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wide">Win Rate</span>
                                <span class="font-bold" :class="card.win_rate >= 50 ? 'text-emerald-500' : 'text-orange-500'">
                                    {{ card.win_rate }}%
                                </span>
                            </div>
                        </div>
                ...
                <div class="h-1.5 w-full bg-slate-100 mt-2">
                    <div class="h-full bg-emerald-500 rounded-r-full" :style="{ width: `${card.win_rate}%` }"></div>
                </div>
