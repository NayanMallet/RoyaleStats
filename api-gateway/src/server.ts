...
app.listen(PORT, () => {
    ...
    logger.info(`  - Clash Data Service: ${process.env.CLASH_DATA_SERVICE_URL || 'http://clash-data-service:3004'}`)
    logger.info(`  - Link Service: ${process.env.LINK_SERVICE_URL || 'http://link-service:3005'}`)
    logger.info(`  - Meta Tracker Service: ${process.env.META_TRACKER_SERVICE_URL || 'http://meta-tracker-service:3006'}`)
