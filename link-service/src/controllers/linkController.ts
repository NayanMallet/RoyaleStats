...
export async function createLink(req: Request, res: Response) {
    try {
        ...
        // Create link
        const result = db.prepare('INSERT INTO player_links (user_id, player_tag) VALUES (?, ?)').run(
            userId,
            `#${normalized}`,
        )

        const link = db.prepare('SELECT * FROM player_links WHERE id = ?').get(result.lastInsertRowid) as PlayerLink
...
export async function getLink(req: Request, res: Response) {
    try {
        ...
        const db = getDatabase()
        const link = db.prepare('SELECT * FROM player_links WHERE user_id = ?').get(userId) as PlayerLink | undefined
